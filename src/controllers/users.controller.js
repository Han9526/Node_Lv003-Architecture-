...
export class UsersController {
   ...
   updateUser = async (req, res, next) => {
      try {
         ...
         const updatedUser = await this.usersService.updateUser(res, name, email, description, password, userId);

         return res.status(200).json({ message: '성공적으로 수정', updatedUser: updatedUser });
      } catch (error) {
         next(error);
      }
   };
